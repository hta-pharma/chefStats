...
test_that("bernard works", {
  ...
  results <- barnard_test_(two_by_two_1)
...
#   results <- barnard_test_(two_by_two_2)
...
test_that("Breslow-Day works", {
  ...
  expect_equal(breslowdaytest_(two_by_two_by_k)$p.value, 0.02737539, tolerance = 1e-6)
})
...
test_that("Breslow-Day errors when no events in a subgourp", {
  ...
  expect_error(breslowdaytest_(two_by_two_by_k_2))
})
