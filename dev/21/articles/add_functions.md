...
1.  Consider what type of function you need: `stat_by_strata_by_trt`,
    `stat_by_strata_across_trt` or `stat_across_strata_across_trt` (see
    article [Function
    types](https://hta-pharma.github.io/chefStats/articles/function_types.html).
2.  Decide what information the function needs in order to compute the
    desired results. See section [Interface with
    chef](#interface-with-chef)
3.  Write the function definition (use
    [`use_chefStats()`](https://hta-pharma.github.io/chefStats/reference/use_chefStats.md)
    to help create the template)
4.  Write the appropriate unit-tests for the function (using
...
1.  Decide which function type it is (see article [Function
    types](https://hta-pharma.github.io/chefStats/articles/function_types.html).
...
2.  Call
    ...
    This will produce a template R function that has the correct
    arguments for what is passed by the chef pipeline to
    `stat_by_strata_by_trt`-type functions (see section [Interface with
    chef](#interface-with-chef) for more details. The skeleton function
    will look like this:
...
3.  Remove unneeded arguments. For our specific function, we only need
    ...
    - `...` The elipses always have to be included in any chefStat
      function So we can delete the others arguments. The additional
      arguments will still be passed to the function by the chef
      pipeline, but they will be collected by `...` and not used.
4.  Modify the function definition for our needs. The final function
...
arguments chef makes available to the function.

The following table describes arguments that are passed to **all**
chefStats function:
...
Arguments always passed to chefStats functions {.table .table
.table-bordered .table .table-hover .table-condensed .table-responsive
style="margin-left: auto; margin-right: auto;"}

  
  

The tables below describe the set of arguments chef passes that vary
...
Additional arguments passed to by_strata_by_trt functions {.table .table
.table-bordered .table .table-hover .table-condensed .table-responsive
style="margin-left: auto; margin-right: auto;"}
...
Additional arguments passed to by_strata_across_trt functions {.table
.table .table-bordered .table .table-hover .table-condensed
...
{.table .table .table-bordered .table .table-hover .table-condensed
.table-responsive style="margin-left: auto; margin-right: auto;"}
...
The only rational for combining functions like this is to save compute
time, due to the way chef pipelines are constructed.

Building block function names are always suffixed with an underscore `_`
to indicate they cannot be called from inside a chef pipeline. For
example,
[`n_event_()`](https://hta-pharma.github.io/chefStats/reference/n_event_.md)
is a building block that is used to make
[`n_event()`](https://hta-pharma.github.io/chefStats/reference/n_event.md),
but
[`n_event_()`](https://hta-pharma.github.io/chefStats/reference/n_event_.md)
can also be use to build other functions, such as
[`count_set()`](https://hta-pharma.github.io/chefStats/reference/count_set.md),
because it does not format it’s output for a chef pipeline. Conversely,
[`n_event()`](https://hta-pharma.github.io/chefStats/reference/n_event.md)
does format the output, so it can be use in a chef pipeline, but not as
a building block.
